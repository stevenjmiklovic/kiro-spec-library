 ...
 */
export function knowledgeSyncRoutes(deps: { db: Database; dataDir: string }) {
  ...
  return new Elysia({ prefix: "" })
    ...
    .post("/knowledge-sync/spec", async ({ body, set }) => {
      ...
      recordEvent(db, "knowledge_sync_run");
      if (result.status === "disabled") set.status = 409;
      else if (result.status === "failed") set.status = 502;
      return result;
    });
}
