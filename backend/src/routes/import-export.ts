...
function buildExportPayload(db: Database): ExportPayload {
...
function applyImport(
  ...
  const specs = listSpecs(db, { limit: 10_000, offset: 0 });
  const specBySpecId = new Map<string, SpecRow>();
  ...
  for (const sidecar of sidecars) {
    ...
    if (!specRow) continue; // Cannot apply overlay without a spec row
...
export function importExportRoutes(deps: { db: Database }) {
  const { db } = deps;
