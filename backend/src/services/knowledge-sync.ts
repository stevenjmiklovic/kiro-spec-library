...
import { type SpecRow, findByKey, listSpecs } from "../db/queries/specs.js";
...
function specSourceUri(spec: SpecRow): string {
 ...
 */
const BODY_ARTIFACTS: readonly string[] = [
 ...
 */
export async function readSpecArtifactBodies(
  spec: SpecRow,
  repoPath: string,
  opts: { fileReader?: (path: string) => Promise<string | null> } = {},
  ...
  for (const filename of BODY_ARTIFACTS) {
    ...
    if (text === null) continue; // file simply doesn't exist for this spec
...
type OutgoingRel = { targetSpecId: string; targetRepository: string; type: string };
 ...
 */
export function buildSpecKnowledgeDoc(
  spec: SpecRow,
  overlay: ReturnType<typeof getOverlay>,
  outgoing: OutgoingRel[],
  bodies?: SpecArtifactBodies,
  ...
  const tags = ((): string[] => {
    ...
    try {
      const parsed: unknown = JSON.parse(overlay.tags);
  ...
  lines.push(
    `Progress: ${spec.completed_tasks}/${spec.total_tasks} tasks complete (${spec.progress}%)`,
  );
  if (overlay?.owner || spec.owner) lines.push(`Owner: ${overlay?.owner || spec.owner}`);
  if (overlay?.theme) lines.push(`Theme: ${overlay.theme}`);
  if (tags.length > 0) lines.push(`Tags: ${tags.join(", ")}`);
  if (overlay?.target_release) lines.push(`Target release: ${overlay.target_release}`);
  lines.push("");
  if (overlay?.summary) {
    lines.push("## Summary");
    lines.push("");
  ...
  if (outgoing.length > 0) {
    ...
    for (const rel of outgoing) {
      ...
      lines.push(
        `- The ${spec.spec_id} spec ${rel.type} the ${rel.targetSpecId} spec` +
...
/** Outcome of pushing a single spec's document to the Knowledge Library. */
type PushOutcome =
  | { kind: "added" }
  | { kind: "duplicate" }
 ...
 */
async function pushSpecDocument(
  spec: SpecRow,
  overlay: ReturnType<typeof getOverlay>,
  outgoing: OutgoingRel[],
  bodies: SpecArtifactBodies | undefined,
  ctx: { doFetch: typeof fetch; url: string },
): Promise<PushOutcome> {
  ...
  if (resp.status === 403) {
    let body: AgentDocResponse = {};
    try {
      body = (await resp.json()) as AgentDocResponse;
    } catch {
 ...
 */
function repoPathMap(db: Database, dataDir: string | undefined): Map<string, string> {
 ...
 */
function outgoingFor(db: Database, spec: SpecRow): OutgoingRel[] {
  const out: OutgoingRel[] = [];
  for (const rel of listAllRelationships(db)) {
    if (rel.source_spec_key !== spec.key) continue;
 ...
 */
export async function syncOneSpecToKnowledgeLibrary(
  db: Database,
  specKey: string,
  opts: {
  ...
  const outcome = await pushSpecDocument(
    spec,
    getOverlay(db, spec.key),
    outgoingFor(db, spec),
    bodies,
    ctx,
  );
 ...
 */
export async function syncSpecsToKnowledgeLibrary(
  db: Database,
  opts: {
    fetchImpl?: typeof fetch;
  ...
  for (const s of specs) byKey.set(s.key, s);

  const outgoingByKey = new Map<string, OutgoingRel[]>();
  for (const rel of listAllRelationships(db)) {
    const target = byKey.get(rel.target_spec_key);
    if (!target) continue; // dangling target — skip rather than emit a bad edge
  ...
  for (const spec of specs) {
    ...
    let bodies: SpecArtifactBodies | undefined;
    ...
    if (repoPath) {
      ...
      if (bodies.truncated) result.bodiesTruncated++;
    }
