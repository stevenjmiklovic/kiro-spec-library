...
function computeContentDigest(artifacts: ArtifactInput[]): string {
...
export class ArchiverService {
  ...
  async maybeCreateSnapshot(
    ...
    artifactContents: ArtifactInput[],
    ...
    try {
      ...
      const snapshot: Snapshot = {
        ...
        retentionPolicy: metadata.retentionPolicy as Snapshot["retentionPolicy"],
        purged: false,
      };
  ...
  async retrieveSnapshot(snapshotId: string): Promise<Snapshot> {
    ...
    for (const artifactRow of artifactRows) {
      ...
      try {
        ...
        throw new Error(
          `Cannot read artifact "${artifactRow.name}" for snapshot ${snapshotId}: ${(err as Error).message}`,
        );
      }
  ...
  private async storeArtifacts(
    ...
    for (const artifact of artifacts) {
      ...
      if (verifyHash !== contentHash) {
        throw new Error(
          `Hash mismatch after write for ${artifact.name}: expected ${contentHash}, got ${verifyHash}`,
        );
      }
   ...
   */
  private async discardPartial(snapshotDir: string): Promise<void> {
    try {
      await rm(snapshotDir, { recursive: true, force: true });
    } catch {
      // Best-effort cleanup
