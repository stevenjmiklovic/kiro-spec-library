...
// ─── Build (export) ──────────────────────────────────────────────────────────
export function buildTextExportZip(db) {
    ...
    const rejections = listAllRejections(db)
        .map((row) => {
        const source = index.byKey.get(row.source_spec_key);
        const target = index.byKey.get(row.target_spec_key);
        if (!source || !target)
    ...
    const snapshots = listAllSnapshots(db)
        .map((row) => {
        const spec = index.byKey.get(row.spec_key);
        if (!spec)
            return null;
...
export function applyTextExportZip(db, zipBytes) {
