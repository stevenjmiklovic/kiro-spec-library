...
import { CONFIDENCE_WEIGHTS, MAX_SUGGESTIONS_PER_SPEC, SUGGESTION_THRESHOLD, } from "@kiro-spec-library/shared";
// ─── Text Processing ─────────────────────────────────────────────────────────
 ...
 */
export function extractMarkdownLinks(content, currentSpecKey, knownKeys) {
    const refs = [];
 ...
 */
export function isProximate(a, b) {
    // Same repository
 ...
 */
export function filterRejected(suggestions, rejections) {
 ...
 */
export function generateAll(specs, metadataMap, contentMap = new Map(), rejections = []) {
    if (specs.length < 2)
        return [];
    const candidates = [];
    ...
    for (let i = 0; i < specs.length; i++) {
        ...
        for (let j = i + 1; j < specs.length; j++) {
            ...
            if (shared.length > 0) {
                const confidence = Math.min(1.0, CONFIDENCE_WEIGHTS.shared_tags_base +
