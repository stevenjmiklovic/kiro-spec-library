  ...
  for (const lane of lanes) {
    ...
    for (const column of columns) {
      const inCell = specs
        .filter(
          (spec) =>
            getLaneValue(spec, yAxisField) === lane && getColumnValue(spec, xAxisField) === column,
        )
...
function GraphLegend(): ReactElement {
  return (
    <div className="graph-legend" aria-label="Relationship legend">
      <span>
        <i className="graph-legend__solid" />
        Accepted relationship
      </span>
      <span>
        <i className="graph-legend__dashed" />
        Pending suggestion
      </span>
    </div>
  );
...
function GraphCanvasInner({
  ...
  const nodes = useMemo(
    () =>
      placeGraphNodes(specs, yAxisField, xAxisField).map((node) => ({
        ...node,
        data: {
          ...node.data,
          selected: node.id === selectedKey,
          superseded: supersededKeys.has(node.id),
          onSelect,
        },
      })),
    [specs, selectedKey, onSelect, supersededKeys, yAxisField, xAxisField],
  );
  const edges = useMemo(() => buildGraphEdges(specs, selectedKey), [specs, selectedKey]);
  const handleNodeClick = useCallback(
    (_event: MouseEvent, node: Node) => onSelect?.(node.id),
    [onSelect],
  );
  ...
  return (
    <div className="graph-canvas-container">
      <GraphOverlays specs={specs} yAxisField={yAxisField} xAxisField={xAxisField} />
      <div className="graph-viewport">
        ...
        >
          <Background color={colorMode === "dark" ? "#34323d" : "#d0d0d8"} gap={26} size={1} />
