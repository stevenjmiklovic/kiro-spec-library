    ...
    const sendToLibrary = async () => {
        ...
        try {
            ...
            if (!res.ok || payload.status === "failed") {
                notify.error(payload.error || payload.message || `Could not send spec (HTTP ${res.status}).`);
                return;
            }
