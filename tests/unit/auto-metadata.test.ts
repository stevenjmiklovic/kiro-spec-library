...
describe("auto-metadata", () => {
  ...
  describe("extractImplementationRef", () => {
    test("finds GitHub PR URL in content", () => {
      ...
      expect(extractImplementationRef(contents)).toBe("https://github.com/org/repo/pull/42");
    });
    ...
    test("finds GitHub issue URL", () => {
      ...
      expect(extractImplementationRef(contents)).toBe("https://github.com/org/repo/issues/99");
    });
    ...
    test("falls back to .config.kiro tracking_issue", () => {
      ...
      expect(extractImplementationRef(contents)).toBe("https://jira.example.com/PROJ-123");
    });
    ...
    test("falls back to .config.kiro implementation", () => {
      ...
      expect(extractImplementationRef(contents)).toBe("https://github.com/org/repo/pull/77");
    });
  ...
  describe("extractSummary", () => {
    test("extracts first paragraph from requirements.md", () => {
      ...
      expect(extractSummary(contents)).toBe("This is the first paragraph of the spec.");
    });
    ...
    test("falls back to design.md", () => {
      ...
      expect(extractSummary(contents)).toBe("Architecture overview for the service.");
    });
  ...
  describe("inferTags", () => {
    ...
    test("excludes stop words", () => {
      const tags = inferTags("The very simple and basic test", ".kiro/specs/test", {});
    ...
    test("returns at most 8 tags", () => {
      const lotsOfContent = Array.from({ length: 20 }, (_, i) => `word${i} `.repeat(10)).join(" ");
  ...
  describe("autoPopulate", () => {
    test("orchestrates all extractors", async () => {
      ...
      expect(result.implementationRef).toBe("https://github.com/org/repo/pull/42");
